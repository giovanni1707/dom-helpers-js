 ...
 * id-shortcut.js
 *
 * Integration Layer - Id Shortcut
 * Provides a convenient Id() function wrapper around Elements helper
 *
 * Features:
 * - Simple element access: Id('myButton')
 * - Multiple element retrieval: Id.multiple('btn1', 'btn2')
 * - Required elements: Id.required('header', 'footer')
 * - Wait for dynamic elements: await Id.waitFor('modal')
 * - Bulk updates: Id.update({ btn1: {...}, btn2: {...} })
 * - Property/attribute shortcuts
 *
 * @version 2.3.1
 * @license MIT
 * @author DOM Helpers Team
 * @requires Elements helper from DOM Helpers core
 */
...
(function(root, factory) {
  ...
  if (typeof global.Elements === 'undefined') {
    console.error('[IdShortcut] Elements helper not found. Please load DOM Helpers core first.');
    return null;
  }
   ...
   * @returns {HTMLElement|null} Enhanced element or null if not found
   *
   * @example
  ...
  function Id(id) {
    // Validate input
    if (typeof id !== 'string') {
      console.warn('[IdShortcut] Invalid ID type. Expected string, got:', typeof id);
      return null;
    }
    ...
    if (id === '') {
      console.warn('[IdShortcut] Empty ID string provided');
      return null;
    }
  ...
  // ========================================================================
  // PROPERTY AND ATTRIBUTE SHORTCUTS
  // ========================================================================
